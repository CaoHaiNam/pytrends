...
@router.post("/api/v1/input_text/keywords_ranking", response_model_exclude_none=True, dependencies=[Depends(validate_secret_token)])
async def solve_1(input_request: InputRequest):
    global count
    # print(request.client.host)
    keywords = input_request.keywords
    try:
        output = utils.keyword_ranking(keywords, timeframe='day')
        return output
    except HTTPException as e:
        raise e
    
@router.post("/api/v1/input_file/keywords_ranking", response_model_exclude_none=True, dependencies=[Depends(validate_secret_token)])
